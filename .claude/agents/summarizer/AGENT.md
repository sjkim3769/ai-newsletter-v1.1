...
3. 각 기사에 대해 순서대로:
   ...
   b. 언어 감지: 원문이 영어인 경우 한국어로 전문 번역 (LLM)
   c. 5줄 요약 생성 — 번역된 텍스트(영어 원문) 또는 원문(한국어) 기반 (LLM)
   d. 착시 자기 검증 (LLM) — 번역본과 요약문 비교
   e. 검증 통과 → summaries 목록에 추가
   f. 검증 실패 → 해당 기사 제외, candidates에서 다음 순위 기사로 대체 후 3번으로
...
1. **언어 처리**: 영어 원문은 **전문 번역 후 번역본 기준으로 요약**. 한국어 원문은 바로 요약.
2. **출력 언어**: 원문 언어와 무관하게 **항상 한국어**로 요약
...
### 번역 프롬프트 템플릿 (영어 원문에만 적용)

```
다음 영어 기사를 자연스러운 한국어로 전문 번역하라.
의역 없이 원문의 사실과 뉘앙스를 그대로 유지할 것.

[영어 원문]
{article_content}

번역문만 출력하라. 설명이나 메모 추가 금지.
```
...
[기사 내용]
{translated_or_original_content}
...
**영어 원문의 경우**: 번역본과 요약문을 비교한다 (번역 오류로 인한 착시 포함).
...
- **비용 최적화**: 기사당 최대 3회 LLM 호출 (영어 번역 1회 + 요약 1회 + 검증 1회), 한국어 기사는 최대 2회
- **재생성 시 추가 호출 허용**: 요약 형식 오류 수정 시 1회 추가
- **후보 대체로 인한 추가 호출**: 대체 기사당 최대 3회 (번역 + 요약 + 검증)
